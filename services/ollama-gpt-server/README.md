...
> 디렉터리명은 `ollama-gpt-server`지만 현재 백엔드는 NVIDIA NIM이다(피벗). 리네임은 후순위(인프라).
...
python services/ollama-gpt-server/scripts/gen_proto.py

# 2) 의존성
pip install -r services/ollama-gpt-server/requirements.txt

# 3) 로컬 .env에 NVIDIA_API_KEY 넣고 실행
$env:PYTHONPATH="services/ollama-gpt-server/src"
